-- Align asphalt sample-unit validation with the submitted Chapters 1–3 manuscript.
-- Local review migration only. This file does not apply itself to any hosted project.
begin;
...
begin
  ...
  if s.area_sqm / n not between 137 and 323 then
    raise exception 'Adjust sample layout: each asphalt unit must be approximately 137–323 m² (230 ± 93 m²) when pavement width is below 7.30 m';
  end if;
...
begin
  ...
  if s.width_meters is null
     or (last_m - first_m) * s.width_meters not between 137 and 323 then
    raise exception 'Additional unit area must be 137–323 m² (230 ± 93 m²) when pavement width is below 7.30 m';
  end if;
  if not exists(select 1 from public.profiles
